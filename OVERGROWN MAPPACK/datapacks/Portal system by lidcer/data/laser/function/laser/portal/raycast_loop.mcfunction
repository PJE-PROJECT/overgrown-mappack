...
execute if block ~ ~ ~ #laser:laser_air if entity @s[scores={Portal_Laser_Distance=..100}] unless entity @e[distance=..1,tag=laser_stop,tag=!hovering] unless entity @e[distance=..1,tag=laser_cube,tag=!laser_raycaster,tag=!hovering,tag=!fizzled] positioned ~ ~1 ~ unless entity @e[distance=..1,tag=sbpg.portal] positioned ^ ^-1 ^0.5 run function laser:laser/portal/raycast_loop
